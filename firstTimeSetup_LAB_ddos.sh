...
if [ -f "/var/local/scf/profiles_ddos_device-LAB.conf" ]; then
	echo "Config Merge verify (profiles_ddos_device-LAB) ..  " # https://support.f5.com/csp/article/K81271448
	tmsh load /sys config merge file /var/local/scf/profiles_ddos_device-LAB.conf verify
	wait
	sleep 2
	echo "Merging DoS Profile (DDoS_DeviceLevel)...  "
	tmsh load /sys config merge file /var/local/scf/profiles_ddos_device-LAB.conf
fi
...
if [ "$VersionCheck" == "OLD" ]; then
	...
	if [ -f "/var/local/scf/profiles_ddos_generic-LAB.conf" ]; then
		echo "Config Merge verify (DDoS_Generic-LAB) ..  " # https://support.f5.com/csp/article/K81271448
		tmsh load /sys config merge file /var/local/scf/profiles_ddos_generic-LAB.conf verify
		wait
		sleep 2
		echo "Merging DoS Profile (DDoS_Generic)...  "
		tmsh load /sys config merge file /var/local/scf/profiles_ddos_generic-LAB.conf
	fi
fi
