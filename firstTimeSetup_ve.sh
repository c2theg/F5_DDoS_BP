...
sleep 5s
#----------------------------------------------------------------------------------------------------------------
Version=$(tmsh show /sys version | grep -i "14")
...
if [ ! -z "$Version_NO_WHITESPACE" ]; then
	...
	We HIGHLY recommend you upgrading to 15.1.x as its features IP-Intelligence catagories in AFM FW rules, that allows it to whitelist valid traffic sourcing from a live list of addresses \e[0m
